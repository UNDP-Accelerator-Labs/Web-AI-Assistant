...
def compile_SYS_PROMT(n_keywords=3):
	return f"""
	You are not an AI assistant that responds to a users.
	You are an AI agent that returns a list of simple search queries 
	that will then be used to retrieve data from an embeddings database that stores 
	a list of conversations between a human user and an AI assistant.
	
	You are given a USER_PROMPT and a CONVERSATION for context. 
	The CONVERSATION is provided as a json array of objets.
	From the USER_PROMPT and the CONVERSATION, you infer {n_keywords} associated search queries 
...
def interpret_prompt(**kwargs):
	...
	n_keywords = kwargs.get('n_keywords', 3)
	...
	stream: ChatResponse = chat(
		...
		messages = [
			{
				'role': 'system',
				'content': compile_SYS_PROMT(n_keywords),
			},
			# Few shot prompts
			{
				'role': 'user',
	...
	return json.loads(full_response.strip())[:int(n_keywords)]
